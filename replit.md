...
# Changelog
- July 8, 2025. Completed comprehensive Therapist Portal with advanced features: Event-driven email notification system using MailerSend that sends beautifully designed emails with deep-links to "My Assignments" tab when new homework is assigned. Consolidated UI removes separate Analytics tab - therapists click client names to view all assignments, then click assignments to see detailed "Report Card" analytics. Report Card mirrors user completion view with scoring trends at top, granular performance data below, showing correct words (>70%) in left column and words needing work (<70%) in right column. Enhanced OpenAI functions generate pronunciation insights identifying specific sounds/patterns to practice. Assignment Templates feature with 10 pre-generated templates covering S/R/TH sounds, daily activities, family, emotions, food, medical terms, weather, and technology. Therapists can save custom templates and reuse them across clients. Full HIPAA-compliant role-based authentication and navigation.
- July 5, 2025. Implemented functional phonetic breakdown system using OpenAI instead of traditional syllables. Updated OpenAI prompt to generate true phonetic sounds (e.g., "EL · uh · fuhnt" for "elephant") rather than syllables ("el-e-phant"). Enhanced scoring system with intelligent mapping that ensures 100% color coverage for all phonetic segments using hybrid approach: direct matching, positional mapping, and overall word score fallback. This guarantees every phonetic sound is colored red or green based on pronunciation assessment.
