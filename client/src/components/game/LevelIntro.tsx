...
export function LevelIntro({ level, onStart }: LevelIntroProps) {
  ...
  // Check if this is a premium level (level 8+)
  useEffect(() => {
    // Premium levels are 8 and above (from server/stripe.ts PREMIUM_LEVEL_THRESHOLD)
    if (level.levelNumber >= 8) {
      setShowPremiumCheck(true);
    }
  }, [level.levelNumber]);
  ...
  // Function to continue with free levels
  const handleContinueToFreeLevels = () => {
    setShowPremiumCheck(false);
  };
        ...
        {showPremiumCheck ? (
          ...
          >
            <PremiumLevelAccess 
              ...
              onContinue={handleContinueToFreeLevels} 
            />
          </motion.div>
        ) : (
          <>
