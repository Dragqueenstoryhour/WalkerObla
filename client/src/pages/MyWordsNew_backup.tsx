...
function AssignmentCarousel({ items, assignmentId }: { items: AssignmentItem[], assignmentId: number }) {
  ...
  const updateAssignmentItemMutation = useMutation({
    mutationFn: async ({ itemId, updates }: { itemId: number, updates: any }) => {
      const response = await fetch(`/api/assignments/items/${itemId}`, {
        method: 'PATCH',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify(updates),
      });
      if (!response.ok) throw new Error('Failed to update assignment item');
